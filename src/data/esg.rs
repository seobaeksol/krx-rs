...
pub struct SriBondInfoRecord {
    ...
    /// 상환일
    #[serde(rename = "REDMPT_DD", deserialize_with = "deserialize_optional_krx_date")]
...
pub fn parse_sri_bond_info(response: ApiResponse<SriBondInfoRecord>) -> Result<DataFrame> {
    ...
    for record in records {
        ...
        listing_dates.push(record.listing_date.map(|d| d.format("%Y-%m-%d").to_string()));
        issue_dates.push(record.issue_date.map(|d| d.format("%Y-%m-%d").to_string()));
        redemption_dates.push(record.redemption_date.map(|d| d.format("%Y-%m-%d").to_string()));
