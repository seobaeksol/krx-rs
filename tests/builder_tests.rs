...
fn test_stock_api_kosdaq_daily_builder() {
    ...
    // 체이닝 테스트
    let client2 = create_test_client();
    let _chained_builder = client2.stock().kosdaq_daily().today();
}
...
fn test_stock_api_konex_daily_builder() {
    ...
    // 체이닝 테스트
    let client2 = create_test_client();
    let _chained_builder = client2.stock().konex_daily().date("20231215").today(); // today()가 마지막이므로 덮어쓰기됨
}
...
fn test_index_api_builders() {
    ...
    // KOSPI 지수 빌더
    let _kospi_builder = index_api.kospi_daily().today();
...
fn test_bond_api_builders() {
    ...
    // 일반채권 빌더
    let _bond_builder = bond_api.bond_daily().today();
...
fn test_etp_api_builders() {
    ...
    // ETN 빌더
    let _etn_builder = etp_api.etn_daily().today();
...
fn test_derivative_api_builders() {
    ...
    let _equity_stock_futures_builder = derivative_api.equity_stock_futures_daily().today();
    ...
    let _equity_stock_options_builder = derivative_api.equity_stock_options_daily().today();
...
fn test_general_api_builders() {
    ...
    // 금 빌더
    let _gold_builder = general_api.gold_daily().today();
...
fn test_builder_method_chaining_order() {
    ...
    let client2 = create_test_client();
    let _builder2 = client2.stock().kospi_daily().today().date("20240105"); // date()가 today() 이후에 와도 정상 작동
