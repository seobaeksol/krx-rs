...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 샘플 JSON 파일 읽기
    let sample_stock_data =
        fs::read_to_string("docs/KRX_API_Spec/samples/stk_bydd_trd_20240105.json")?;
    let sample_index_data =
        fs::read_to_string("docs/KRX_API_Spec/samples/krx_dd_trd_20240105.json")?;
    ...
    match serde_json::from_str::<krx_rs::data::ApiResponse<krx_rs::data::stock::KospiDailyRecord>>(
        &sample_stock_data,
    ) {
        Ok(response) => {
            println!(
                "✅ Successfully parsed {} stock records",
                response.data.len()
            );
    ...
    match serde_json::from_str::<krx_rs::data::ApiResponse<krx_rs::data::index::KrxIndexDailyRecord>>(
        &sample_index_data,
    ) {
        Ok(response) => {
            println!(
                "✅ Successfully parsed {} index records",
                response.data.len()
            );
